...
class BoW(TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None):
        ...
        self.bow = list(map(lambda item: item[0], counter.most_common(self.k)))
...
class TfIdf(TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None):
        ...
        for term, df in df_counter.most_common(self.k):
    ...
    def _text_to_tf_idf(self, text: str) -> np.ndarray:
        ...
        *Note* don't forget to normalize, when self.normalize == True
        ...
        counter_tf = Counter(text.split())
        result = [counter_tf.get(token, 0) / idf for token, idf in self.idf.items()]        
